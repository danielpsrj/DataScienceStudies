...
def generate_clustering_data(
    ...
    for cluster_id in range(n_clusters):
        ...
        cluster_samples = (
            np.random.randn(samples_per_cluster, 2) * cluster_std + centers[cluster_id]
        )
    ...
    if remainder > 0:
        ...
        remainder_samples = (
            np.random.randn(remainder, 2) * cluster_std + centers[cluster_id]
        )
...
def generate_complex_clustering_data(
    ...
    if dataset_type == "moons":
        ...
        X, y_true = make_circles(
            n_samples=n_samples, noise=noise, random_state=random_state, factor=0.5
        )
        ...
        X, y_true = make_blobs(
            n_samples=n_samples, centers=4, random_state=random_state
        )
...
def visualize_clusters_2d(
    ...
    df = pd.DataFrame({"x": X[:, 0], "y": X[:, 1], "cluster": labels.astype(str)})
